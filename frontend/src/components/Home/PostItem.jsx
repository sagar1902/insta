...
const PostItem = ({ _id, caption, likes, comments, image, postedBy, savedBy, createdAt, setUsersDialog, setUsersList }) => {
    ...
    return (
        <div className="flex flex-col border rounded bg-white relative" id='dfsdfsdfsf'>
            ...
            <div className="relative flex items-center justify-center" onDoubleClick={setLike}>
                <img draggable="false" loading="lazy" className="w-full h-full object-cover object-center" src={`http://localhost:4000/api/postS3Config/${image}`} alt={image} />
                {likeEffect &&
                    ...
                    <ScrollToBottom className="w-full h-52 overflow-y-auto py-1">
                        {allComments.map((c) => (
                            <div className="flex items-start mb-2 space-x-2" key={c._id}>
                                <img draggable="false" className="h-7 w-7 rounded-full object-cover mr-0.5" src={`http://localhost:4000/api/postS3Config/${c.user.avatar}`} alt="avatar" />
